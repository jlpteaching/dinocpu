...
class PipelinedCPUBP(implicit val conf: CPUConfig) extends BaseCPU {
  ...
  id_ex_ctrl.io.in.ex.prediction := predictor.io.prediction
  ...
  id_ex_ctrl.io.in.m.taken    := DontCare
  ...
  when (id_ex_ctrl.io.data.ex.branch && ~hazard.io.exmem_bubble) {
    ...
    when (id_ex_ctrl.io.data.ex.prediction === branchCtrl.io.taken) {
  ...
  when (id_ex_ctrl.io.data.ex.jump(1)) {
    // If the high bit in the jump control is set, then we jump no matter what.
    when (id_ex_ctrl.io.data.ex.jump(0) === 0.U) {
    ...
    when (branchCtrl.io.taken =/= id_ex_ctrl.io.data.ex.prediction && id_ex_ctrl.io.data.ex.branch) {
  ...
  registers.io.wen       := mem_wb_ctrl.io.data.regwrite && (registers.io.writereg =/= 0.U)
