...
class AsyncScratchPadMemory(val num_core_ports: Int,val num_bytes: Int = (1 << 21)) extends Module
{
   val io = IO(new Bundle
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = 32)) )
...
class SyncScratchPadMemory(val num_core_ports: Int, val num_bytes: Int = (1 << 21)) extends Module
{
   val io = IO(new Bundle
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = 32)) )
