// Contains the driver class responsible for performing instruction tests.
...
object CPUTesterDriver {
  def apply(testCase: CPUTestCase, cpuType: String, branchPredictor: String = "", memType: String = "combinational",
            memPortType: String = "combinational-port"): Boolean = {
    val cpustr = if (branchPredictor != "") { cpuType+"-bp" } else { cpuType }

    val driver = new CPUTesterDriver(cpustr, branchPredictor, testCase.binary, testCase.extraName, false, memType, memPortType)
